...
| `tests/daily-briefing.spec.ts` | The new-day dialog. Above all: dismissing it writes a SERVER column, so it stays closed across a reload and on every other device — a localStorage implementation passes every unit test in the repo and fails this. Also that an account with nothing scheduled is never interrupted, that the endpoint answers in full with the LLM switched off, and that acknowledging keeps the FIRST timestamp |
...
suite: `tests/` currently holds 37 spec files. Run `ls tests/` for the rest.
